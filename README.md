...
**注意：** 在实际运行的过程中直接运行上述指令会消耗极大的内存，大约200G。分步运行的指令如下。每一步运行大约也要消耗100G的内存，如果内存还是不够需要修改代码将日数据分批处理，后续程序也可能出现这个问题。

```bash
python table_pro.py --adj
python table_pro.py --min
python table_pro.py --day
python table_pro.py --fix
```
...
```
python prediction.py --start 2017-01-01 --end 2017-06-31 --model 2016-12-32 --type 0
```
...
全部的训练数据从2016-2023年，回测的时间段为2017-2023年。在第一次训练的过程中使用2016-01-01到2016-12-31的数据作为训练集和验证集，对2017-01-01到2017-06-30的数据进行回测。第二次训练的过程使用2016-01-01到2017-06-30的数据作为训练集和验证集，对2017-07-01到2017-12-31的数据进行回测。以此类推一直将所有的数据回测完。
